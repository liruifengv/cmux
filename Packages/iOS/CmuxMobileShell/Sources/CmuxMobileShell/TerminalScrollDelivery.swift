...
/// Pure routing decision for a phone scroll gesture.
///
/// Primary screen: the phone's local Ghostty mirror owns the viewport, so no
/// scroll delta is ever sent to the Mac; the only RPC is a `delta_lines = 0`
/// scrollback-window fetch when the prefetch state says the local history
/// needs (re)priming or deepening. Alternate screen: the wheel must reach the
/// real PTY, so the delta is forwarded unchanged.
