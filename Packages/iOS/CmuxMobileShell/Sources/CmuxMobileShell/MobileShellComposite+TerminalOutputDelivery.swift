...
extension MobileShellComposite {
    ...
    func recordTerminalRenderGridDelivery(_ renderGrid: MobileTerminalRenderGridFrame) {
        ...
        let establishesBaseline = renderGrid.full || renderGrid.isReplaceableViewportPatchForMobileDelivery
        if renderGrid.activeScreen == .alternate, establishesBaseline {
    ...
    func deliverAuthoritativeTerminalRenderGrid(
        ...
        let needsRenderGridBaseline = (terminalOutputTransport == .renderGrid && !hasDeliveredSeq)
            || (
        ...
        let establishesRenderGridBaseline = renderGrid.full || renderGrid.isReplaceableViewportPatchForMobileDelivery
        ...
        let bypassLiveFullFrameBarrier = source == "event"
            && renderGrid.full
            && (
        ...
        guard deliverTerminalRenderGrid(
            ...
            bypassReplayBarrier: bypassLiveFullFrameBarrier
        ) else { return }
        if bypassLiveFullFrameBarrier,
