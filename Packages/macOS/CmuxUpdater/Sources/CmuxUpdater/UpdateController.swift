...
public final class UpdateController {
    ...
    private let backgroundRetryLimit = 3
    private let backgroundRetryDelay: Duration = .seconds(5 * 60)
    /// How often the deferred "restart when idle" loop re-evaluates the host's idle signal.
    private let restartWhenIdlePollInterval: Duration = .seconds(30)
    ...
    private func scheduleBackgroundRetryIfTransient(_ error: any Error) {
    ...
    private func cancelRestartWhenIdleLoop() {
        restartWhenIdleTask?.cancel()
