...
class TerminalController {
    ...
    private nonisolated func socketWorkerV1ResponseIfHandled(cmd: String, args: String) -> (handled: Bool, response: String?) {
        ...
        return withSocketCommandPolicy(commandKey: cmd, isV2: false, params: policyParams) {
            ...
            case "ping":
                return (true, "PONG")
            default:
                // Keep `socketWorkerV1Commands` and this switch in lockstep: a
                // policy entry without a worker body here falls back to the
                // main-lane dispatcher instead of failing.
                return (false, nil)
            }
        }
    }
    ...
    nonisolated func v2MainSync<T>(commandKey: String? = nil, _ body: @MainActor () -> T) -> T {
        ...
        // `withSocketCommandPolicy` on this thread). The fast path with
        // signposts disabled costs two clock reads plus, in DEBUG, one
        // thread-local lookup for the per-command accumulator; all string
        // formatting stays behind the signpost-enabled check.
